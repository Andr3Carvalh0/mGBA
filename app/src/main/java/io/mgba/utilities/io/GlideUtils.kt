...
class GlideUtils<T> private constructor(private val holder: T, private val url: String) {
    ...
    fun colorView(primarySwatch: Colors, secundarySwatch: Colors, view: List<View>): GlideUtils<*> {
        view.map { v ->  ColorWrapper(v, primarySwatch, secundarySwatch)}
            .forEach { v -> views.add(v) }
        return this
    }

    fun colorViewWithCustomBackground(primarySwatch: Colors, secundarySwatch: Colors, view: List<View>): GlideUtils<*> {
        view.map { v ->  CustomBackgroundColorWrapper(v, primarySwatch, secundarySwatch)}
            .forEach { v -> views.add(v) }
        return this
    }

    fun colorView(swatch: Colors, title: Boolean, view: List<TextView>): GlideUtils<*> {
        view.map { v ->  TextColorWrapper(swatch, v, title)}
            .forEach { v -> views.add(v) }
        return this
    }
    ...
    private fun processColorExtraction(rm: RequestBuilder<Drawable>) {
        ...
        requestColor.intoCallBack { palette -> views.forEach { v -> v.colorView(palette, requestColor) } }
    ...
    private fun getPalette(v: Colors, palette: Palette?): Palette.Swatch {
        when (v) {
            Colors.MUTED -> return palette!!.mutedSwatch!!
            Colors.VIBRANT -> return palette!!.vibrantSwatch!!
            Colors.LIGHT_MUTED -> return palette!!.lightMutedSwatch!!
            Colors.LIGHT_VIBRANT -> return palette!!.lightVibrantSwatch!!
            Colors.DARK_MUTED -> return palette!!.darkMutedSwatch!!
